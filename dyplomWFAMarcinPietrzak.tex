...
W ButterflyActor najważniejszą metodą jest UpdatePosition(FVector position), w której poprzez rozwiązanie wcześniej pokazanych równań różniczkowych po przecałkowaniu. Następnie odpowiednio ustawiamy te równania dla pozycji X, Y i Z danego aktora \cite{motyle_cpp}

\lstinputlisting[caption=Metoda UpdatePosition(), label={lst:listing-cpp}, language=C++]{code/butterfly/ButterflyActorTick.cpp}
...
\newpage
\section{Sterowanie postacią w grze i mapa}
...
Oświadczam, że załączona przeze mnie wersja elektroniczna pracy dyplomowej w systemie
Archiwum Prac Dyplomowych:
\begin{enumerate}
 \item jest wersją ostateczną złożoną w celu przystąpienia do egzaminu dyplomowego w
Uniwersytecie Wrocławskim;
 \item jest mojego autorstwa i nie narusza autorskich praw w rozumieniu ustawy z dnia 4
lutego 1994 r. o prawie autorskim i prawach pokrewnych (tekst jednolity: Dz. U.
z 2021 r., poz.1062);
 \item nie zawiera danych i informacji uzyskanych w sposób niedozwolony;
 \item nie była wcześniej przedmiotem innej urzędowej procedury związanej z nadaniem
dyplomu uczelni wyższej lub tytułu zawodowego.
\end{enumerate}
\end{document}
